...
Run it:
```bash
scafctl run workflow -f hello.yaml
```
...
```bash
# Basic execution (shows progress by default)
scafctl run workflow -f my-workflow.yaml

# Run without progress output (for scripts/pipelines)
scafctl run workflow -f my-workflow.yaml --no-progress -o json

# Override resolver values
scafctl run workflow -f my-workflow.yaml -r environment=prod

# Limit parallel actions
scafctl run workflow -f my-workflow.yaml --max-concurrency=4

# Dry-run (show plan without executing)
scafctl run workflow -f my-workflow.yaml --dry-run
```
...
scafctl render workflow -f my-workflow.yaml

# Render to YAML
scafctl render workflow -f my-workflow.yaml --output=yaml

# Write to file
scafctl render workflow -f my-workflow.yaml --output-file=graph.json
```
