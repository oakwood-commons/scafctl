...
### Phase 11: CLI Integration ✅ COMPLETED

**Status:** Completed on 2026-01-29

**Goal:** Add `run workflow` and `render workflow` subcommands

**Files created:**
- [x] `pkg/cmd/scafctl/render/render.go` - Render command root
- [x] `pkg/cmd/scafctl/render/workflow.go` - Render workflow subcommand
- [x] `pkg/cmd/scafctl/run/workflow.go` - Run workflow subcommand

**Files modified:**
- [x] `pkg/cmd/scafctl/root.go` - Added render command registration
- [x] `pkg/cmd/scafctl/run/run.go` - Added workflow subcommand

**Commands implemented:**

```bash
# Render executor-ready graph
scafctl render workflow <file> [flags]
...
scafctl run workflow <file> [flags]
  --dry-run              # Show what would be executed
  --max-concurrency=N    # Limit parallel actions (default: 4)
  --no-progress          # Disable progress output (progress is on by default)
  -o quiet|json|yaml     # Output format (default: quiet)
  -r key=value        # Override resolver/input values
```

**Key implementation details:**
- Registry adapter pattern for interface compatibility between action and resolver systems
- `workflowRegistryAdapter` wraps resolver registry for action system (converts `(Provider, error)` → `(Provider, bool)`)
- `resolverRegistryAdapter` wraps for resolver executor (passes through `(Provider, error)`)
- Resolver execution runs first to populate data context
- Action graph built from spec.Workflow using resolver results
- Dry-run mode shows execution plan without side effects
- Progress callback integration for real-time status updates
- JSON and YAML output format support

**Types implemented:**

```go
// pkg/cmd/scafctl/render/workflow.go

type WorkflowOptions struct {
    OutputFormat string   // json or yaml
    OutputFile   string   // optional file path
    SetValues    []string // key=value overrides
}

// Adapters for registry interface compatibility
type registryAdapter struct { ... }      // For action system
type resolverRegistryAdapter struct { ... } // For resolver system

// pkg/cmd/scafctl/run/workflow.go
...
// WorkflowProgressCallback implements ProgressCallback interface
