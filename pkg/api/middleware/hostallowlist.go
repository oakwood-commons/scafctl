...
//   - A leading "*." entry matches any single-or-multi label subdomain as well as
//     the bare domain, e.g. "*.example.com" matches "api.example.com" and
//     "example.com".
...
func HostAllowlist(allowedHosts []string, lgr logr.Logger) func(http.Handler) http.Handler {
	...
	// A non-empty config that normalizes to nothing means the operator intended
	// to restrict but did not. Silent fail-open is acceptable here; an
	// UNDETECTABLE one is not, so say so at startup.
	if len(allowedHosts) > 0 && disabledReason != "" {
		lgr.Info("WARNING: host allowlist is configured but disabled; every Host header will be accepted",
...
func normalizeAllowedHosts(allowedHosts []string) (normalized []string, disabledReason string) {
	...
	for _, h := range allowedHosts {
		...
		if h == "*" {
			return nil, "wildcard entry \"*\" accepts all hosts"
		}
		if h == "*." {
			continue
		}
