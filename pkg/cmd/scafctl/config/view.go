...
func (o *ViewOptions) Run(ctx context.Context) error {
	...
	// Route through the fail-closed SanitizeConfig allowlist before rendering
	// so on-disk secrets (auth client secrets, TLS key paths, opaque plugin
	// handler maps, resolver headers) never reach stdout.
	sanitized := appconfig.SanitizeConfig(cfg)
	cfgAny, err := kvx.StructToMap(sanitized)
	if err != nil {
