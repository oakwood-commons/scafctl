...
func (o *SolutionOptions) runActionGraph(ctx context.Context, lgr logr.Logger) error {
	...
	if sol.Spec.HasResolvers() {
		...
		ctx, err = o.loadStateIntoContext(ctx, sol, reg, params, "render solution")
		if err != nil {
...
func (o *SolutionOptions) runSnapshot(ctx context.Context, lgr logr.Logger) error {
	...
	// State lifecycle: load persisted state before resolver execution so
	// the state provider can serve previously saved values (read-only).
	ctx, err = o.loadStateIntoContext(ctx, sol, o.getRegistry(ctx), params, "render solution")
	if err != nil {
		return o.exitWithCode(err, exitcode.GeneralError)
	}
...
func (o *SolutionOptions) loadStateIntoContext(ctx context.Context, sol *solution.Solution, reg *provider.Registry, params map[string]any, subcommand string) (context.Context, error) {
	if sol.State == nil {
		return ctx, nil
	}
