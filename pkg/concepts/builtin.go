...
**Save-time format and emit targets** — 'backend.format' controls what shape of the state document a backend receives at save time: 'full' (default) saves everything (resolvers, fingerprints, command); 'intent' saves a lean, deterministic projection (schemaVersion, metadata.solution/version, parameters, attestation) with no timestamps -- suitable for committing and signing. 'emit' is a list of additional save-only backends, each independently formatted and independently gated by its own 'enabled' (evaluated at save time, so it may reference any resolver). This lets one solution keep a full-fidelity primary state file locally while also publishing a lean intent document. Setting 'format: intent' directly on the PRIMARY backend (with no emit) is a supported but lossy shortcut: immutable locks live in the omitted 'resolvers' section, so 'scafctl lint' warns ('state-format-lossy-with-immutable') when that combination coexists with an 'immutable: true' resolver.

**--state-file** — points 'run solution'/'run resolver'/'run action' at an explicit state file, overriding the solution's own state block (inheriting its declared format) or enabling state when none is configured. Because state decoding tolerates a lean document, an 'intent'-format file (schemaVersion + metadata + parameters only) is a valid --state-file input; scafctl regenerates the full document in place at save time (or stays lean, if the inherited format is 'intent').`,
		Examples: []string{
			...
			"# Full-fidelity primary state locally, lean signable intent published via emit.\nstate:\n  enabled: true\n  backend:\n    provider: file\n    inputs:\n      path: \".scafctl/state.json\"\n  emit:\n    - provider: file\n      format: intent\n      inputs:\n        path: \"intent/sandbox.json\"",
		},
