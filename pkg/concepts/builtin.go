...
**Configuration** -- add a top-level 'state' block to the solution. Reading and writing are configured separately:
...
- **load** -- where state is READ from before resolvers run: a provider (e.g., file, http) and its inputs. Load never writes.
- **save** -- the list of places state is WRITTEN after a successful run. It is the ONLY write mechanism: nothing is saved unless a save target says so. Each target names its own provider and inputs, or uses 'extends: load' to write back to the load block's provider and inputs (its own inputs merged on top, so it can load from one location and save to another -- e.g. a GitHub target loading from main and committing to a feature branch).

Both are optional: load + save persists across runs; load only is a read-only replay; save only starts from empty state each run; neither does nothing (lint 'empty-state-config').

**Dynamic configuration** -- 'enabled' and 'load.inputs' may reference resolver outputs (rslvr:/expr:/tmpl: with '_.<name>'), so the state file path or the enable flag can be computed from resolved values. Because these fields are evaluated *before* state is loaded, scafctl first runs a minimal "Phase A" -- just the referenced resolvers and their transitive dependencies -- evaluates the fields, loads state, then reuses those Phase-A results in the main run (they are not executed twice). Only *state-independent* resolvers may be referenced: a resolver that reads state (via the 'state' provider) or depends on one that does would be circular and is rejected at load time (lint rules 'state-ref-state-dependent' and 'state-ref-unknown'). Save targets are evaluated after resolvers run and may reference any resolver.

**Lifecycle**:
1. Before resolvers run, the state manager calls the load provider with 'state_load' to read any existing state, then merges the saved parameters with the current CLI parameters (CLI values win on conflict).
2. Resolvers execute normally -- the parameter provider returns the merged (replayed) values -- and immutable values are verified against their locks.
3. For run solution / run action, save targets marked 'checkpoint: true' are written before workflow actions run, so immutable values locked this run survive a later action failure.
4. After a successful run, every enabled save target is written with 'state_save': the merged parameter set plus the locked values of any resolvers marked 'immutable: true'.

**Immutable resolvers** -- mark a resolver 'immutable: true' to lock its resolved value in state after the first run. On later runs the resolver still executes, but its value is compared against the stored value and execution fails if they differ. Use this for non-deterministic values (e.g., UUIDs) that must stay stable across runs. Locks live only in a full-format document, so a solution with immutable resolvers needs a load block and a full-format save target (lint 'state-requires-load' and 'state-requires-full-save').

**Providers** -- file (local JSON; relative paths resolve against the current working directory), http (remote REST API). External providers (e.g., github) can be installed as plugins. Each implements CapabilityState with state_load, state_save, and state_delete operations.

**Save format and parameter narrowing** -- a save target's 'format' controls what shape of the state document it receives: 'full' (default) saves everything (resolvers, fingerprints, command); 'intent' saves a lean, deterministic projection (schemaVersion, metadata.solution/version, parameters, attestation) with no timestamps -- suitable for committing and signing. An intent target's 'parameters.include' / 'parameters.exclude' narrow which parameters it carries, so run-control parameters stay out of a committed intent.

**CLI overrides** -- each flag replaces exactly one half of the configuration. '--state-file PATH' replaces the load block with a read of an existing file; the declared save targets still run, and an 'extends: load' target keeps writing to the load location the solution declared, so the file passed in is never overwritten. '--state-output PATH' replaces every save target with one full-format file. Both force state on for the run (a notice says so when the solution's 'enabled' could have turned it off), and relative paths resolve against the invoking working directory. '--no-state-output' disables saving (immutable values are still verified). '--no-state' disables state entirely. Replaying a document that has parameters but no immutable locks (such as an intent document) through a solution with immutable resolvers fails unless '--allow-missing-locks' is given, because those values would be re-derived and any saved locks replaced.`,
		Examples: []string{
			"# Persist state in a local file; parameters replay automatically\nstate:\n  enabled: true\n  load:\n    provider: file\n    inputs:\n      path: \"my-app-state.json\"\n  save:\n    - extends: load   # write back to the same file (full format)\n\nspec:\n  resolvers:\n    # Locked after the first run -- value is saved and verified on later runs\n    deployment_id:\n      type: string\n      immutable: true\n      resolve:\n        with:\n          - provider: parameter\n            inputs:\n              key: \"deployment_id\"\n    # Mutable -- replayed from saved parameters, overridable via -r\n    region:\n      type: string\n      resolve:\n        with:\n          - provider: parameter\n            inputs:\n              key: \"region\"\n          - provider: static\n            inputs:\n              value: \"us-west-2\"",
			...
			"# Full state locally, plus a lean signable intent with a run-control param narrowed out.\nstate:\n  enabled: true\n  load:\n    provider: file\n    inputs:\n      path: \".scafctl/state.json\"\n  save:\n    - extends: load\n    - provider: file\n      format: intent\n      parameters:\n        exclude: [\"mode\"]\n      inputs:\n        path: \"intent/sandbox.json\"",
			"# GitHub: load from main, save to the PR branch.\nstate:\n  load:\n    provider: github\n    inputs:\n      owner: { literal: acme }\n      repo: { literal: infra }\n      path: { literal: state/app.json }\n      ref: { literal: main }\n  save:\n    - extends: load\n      inputs:\n        branch: { rslvr: featureBranch }",
		},
...
- **_** -- map of resolved resolver values (_.region). Available in resolve inputs/when, transform, validate, and action when/inputs, and in the state 'enabled'/'load.inputs' fields (where it holds the Phase-A resolver outputs). Referencing _.other also creates an implicit dependency edge.
...
- **__params** -- CLI parameters (-r key=value), available in state configuration expressions only (state.enabled, state.load.inputs, and save target inputs and enabled), in CEL and Go templates. Load-time fields see the raw CLI parameters; save targets see the merged (replayed plus CLI) set. Unlike _ (resolver outputs), __params is always populated.
