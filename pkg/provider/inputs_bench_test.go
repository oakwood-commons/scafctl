package provider

import (
	"context"
	"testing"

	"github.com/oakwood-commons/scafctl/pkg/celexp"
	"github.com/oakwood-commons/scafctl/pkg/gotmpl"
)

func BenchmarkInputResolver_Literal(b *testing.B) {
	ctx := context.Background()
	schema := SchemaDefinition{
		Properties: map[string]PropertyDefinition{
			"name":    {Type: PropertyTypeString, Required: true},
			"count":   {Type: PropertyTypeInt, Required: true},
			"enabled": {Type: PropertyTypeBool, Required: true},
		},
	}

	inputs := map[string]any{
		"name":    InputValue{Literal: "test-name"},
		"count":   InputValue{Literal: 42},
		"enabled": InputValue{Literal: true},
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		resolver := NewInputResolver(ctx, schema)
		_, err := resolver.ResolveInputs(inputs)
		if err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkInputResolver_Rslvr(b *testing.B) {
	resolverCtx := map[string]any{
		"environment": "prod",
		"region":      "us-east",
		"count":       100,
	}
	ctx := WithResolverContext(context.Background(), resolverCtx)

	schema := SchemaDefinition{
		Properties: map[string]PropertyDefinition{
			"env":    {Type: PropertyTypeString, Required: true},
			"region": {Type: PropertyTypeString, Required: true},
			"count":  {Type: PropertyTypeInt, Required: true},
		},
	}

	inputs := map[string]any{
		"env":    InputValue{Rslvr: "environment"},
		"region": InputValue{Rslvr: "region"},
		"count":  InputValue{Rslvr: "count"},
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		resolver := NewInputResolver(ctx, schema)
		_, err := resolver.ResolveInputs(inputs)
		if err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkInputResolver_RslvrNested(b *testing.B) {
	resolverCtx := map[string]any{
		"config": map[string]any{
			"database": map[string]any{
				"host": "localhost",
				"port": 5432,
			},
		},
	}
	ctx := WithResolverContext(context.Background(), resolverCtx)

	schema := SchemaDefinition{
		Properties: map[string]PropertyDefinition{
			"host": {Type: PropertyTypeString, Required: true},
			"port": {Type: PropertyTypeInt, Required: true},
		},
	}

	inputs := map[string]any{
		"host": InputValue{Rslvr: "config.database.host"},
		"port": InputValue{Rslvr: "config.database.port"},
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		resolver := NewInputResolver(ctx, schema)
		_, err := resolver.ResolveInputs(inputs)
		if err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkInputResolver_CEL(b *testing.B) {
	resolverCtx := map[string]any{
		"environment": "prod",
		"count":       10,
	}
	ctx := WithResolverContext(context.Background(), resolverCtx)

	schema := SchemaDefinition{
		Properties: map[string]PropertyDefinition{
			"cluster": {Type: PropertyTypeString, Required: true},
			"scaled":  {Type: PropertyTypeInt, Required: true},
		},
	}

	inputs := map[string]any{
		"cluster": InputValue{Expr: celexp.Expression(`environment + "-cluster"`)},
		"scaled":  InputValue{Expr: celexp.Expression("count * 2")},
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		resolver := NewInputResolver(ctx, schema)
		_, err := resolver.ResolveInputs(inputs)
		if err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkInputResolver_Template(b *testing.B) {
	resolverCtx := map[string]any{
		"environment": "prod",
		"region":      "us-east",
	}
	ctx := WithResolverContext(context.Background(), resolverCtx)

	schema := SchemaDefinition{
		Properties: map[string]PropertyDefinition{
			"url": {Type: PropertyTypeString, Required: true},
		},
	}

	inputs := map[string]any{
		"url": InputValue{Tmpl: gotmpl.GoTemplatingContent("https://{{.environment}}.{{.region}}.example.com")},
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		resolver := NewInputResolver(ctx, schema)
		_, err := resolver.ResolveInputs(inputs)
		if err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkInputResolver_TypeCoercion(b *testing.B) {
	tests := []struct {
		name   string
		inputs map[string]any
		schema SchemaDefinition
	}{
		{
			name: "string to int",
			inputs: map[string]any{
				"count": InputValue{Literal: "42"},
			},
			schema: SchemaDefinition{
				Properties: map[string]PropertyDefinition{
					"count": {Type: PropertyTypeInt, Required: true},
				},
			},
		},
		{
			name: "string to bool",
			inputs: map[string]any{
				"enabled": InputValue{Literal: "true"},
			},
			schema: SchemaDefinition{
				Properties: map[string]PropertyDefinition{
					"enabled": {Type: PropertyTypeBool, Required: true},
				},
			},
		},
		{
			name: "string to array",
			inputs: map[string]any{
				"tags": InputValue{Literal: "tag1,tag2,tag3"},
			},
			schema: SchemaDefinition{
				Properties: map[string]PropertyDefinition{
					"tags": {Type: PropertyTypeArray, Required: true},
				},
			},
		},
		{
			name: "int to string",
			inputs: map[string]any{
				"id": InputValue{Literal: 42},
			},
			schema: SchemaDefinition{
				Properties: map[string]PropertyDefinition{
					"id": {Type: PropertyTypeString, Required: true},
				},
			},
		},
	}

	for _, tt := range tests {
		b.Run(tt.name, func(b *testing.B) {
			ctx := context.Background()
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				resolver := NewInputResolver(ctx, tt.schema)
				_, err := resolver.ResolveInputs(tt.inputs)
				if err != nil {
					b.Fatal(err)
				}
			}
		})
	}
}

func BenchmarkInputResolver_ComplexSchema(b *testing.B) {
	resolverCtx := map[string]any{
		"environment": "prod",
		"region":      "us-east",
		"config": map[string]any{
			"database": map[string]any{
				"host": "localhost",
				"port": 5432,
			},
		},
		"count": 10,
	}
	ctx := WithResolverContext(context.Background(), resolverCtx)

	schema := SchemaDefinition{
		Properties: map[string]PropertyDefinition{
			"name":        {Type: PropertyTypeString, Required: true},
			"env":         {Type: PropertyTypeString, Required: true},
			"region":      {Type: PropertyTypeString, Required: true},
			"dbHost":      {Type: PropertyTypeString, Required: true},
			"dbPort":      {Type: PropertyTypeInt, Required: true},
			"url":         {Type: PropertyTypeString, Required: true},
			"count":       {Type: PropertyTypeInt, Required: true},
			"scaledCount": {Type: PropertyTypeInt, Required: true},
			"enabled":     {Type: PropertyTypeBool, Required: true},
			"tags":        {Type: PropertyTypeArray, Required: true},
		},
	}

	inputs := map[string]any{
		"name":        InputValue{Literal: "my-app"},
		"env":         InputValue{Rslvr: "environment"},
		"region":      InputValue{Rslvr: "region"},
		"dbHost":      InputValue{Rslvr: "config.database.host"},
		"dbPort":      InputValue{Rslvr: "config.database.port"},
		"url":         InputValue{Tmpl: gotmpl.GoTemplatingContent("https://{{.environment}}.{{.region}}.example.com")},
		"count":       InputValue{Rslvr: "count"},
		"scaledCount": InputValue{Expr: celexp.Expression("count * 2")},
		"enabled":     InputValue{Literal: true},
		"tags":        InputValue{Literal: "web,api,production"},
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		resolver := NewInputResolver(ctx, schema)
		_, err := resolver.ResolveInputs(inputs)
		if err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkInputResolver_SecretMasking(b *testing.B) {
	ctx := context.Background()
	schema := SchemaDefinition{
		Properties: map[string]PropertyDefinition{
			"apiKey": {Type: PropertyTypeString, Required: true, IsSecret: true},
			"name":   {Type: PropertyTypeString, Required: true},
		},
	}

	inputs := map[string]any{
		"apiKey": InputValue{Literal: "secret-api-key-12345"},
		"name":   InputValue{Literal: "my-app"},
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		resolver := NewInputResolver(ctx, schema)
		_, err := resolver.ResolveInputs(inputs)
		if err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkMaskValue(b *testing.B) {
	tests := []struct {
		name     string
		value    any
		isSecret bool
	}{
		{
			name:     "mask string secret",
			value:    "secret-password-123",
			isSecret: true,
		},
		{
			name:     "no mask public string",
			value:    "public-value",
			isSecret: false,
		},
		{
			name:     "mask int secret",
			value:    42,
			isSecret: true,
		},
	}

	for _, tt := range tests {
		b.Run(tt.name, func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				_ = MaskValue(tt.value, tt.isSecret)
			}
		})
	}
}

func BenchmarkCoerceType(b *testing.B) {
	resolver := &InputResolver{}

	tests := []struct {
		name       string
		value      any
		targetType PropertyType
	}{
		{
			name:       "string to int",
			value:      "42",
			targetType: PropertyTypeInt,
		},
		{
			name:       "int to string",
			value:      42,
			targetType: PropertyTypeString,
		},
		{
			name:       "string to bool",
			value:      "true",
			targetType: PropertyTypeBool,
		},
		{
			name:       "string to array",
			value:      "a,b,c,d,e",
			targetType: PropertyTypeArray,
		},
		{
			name:       "no coercion needed",
			value:      map[string]any{"key": "value"},
			targetType: PropertyTypeAny,
		},
	}

	for _, tt := range tests {
		b.Run(tt.name, func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				_, err := resolver.coerceType("test", tt.value, tt.targetType)
				if err != nil {
					b.Fatal(err)
				}
			}
		})
	}
}
