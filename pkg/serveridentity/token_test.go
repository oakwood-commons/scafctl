...
func TestTokenCache_GetSet_RoundTrip(t *testing.T) {
	...
	tc := NewTokenCache[string, testTokenResult](ctx, 10, 0, time.Hour)

	token := testTokenResult{
		AccessToken: "abc123",
		ExpiresIn:   3600,
	}
...
func TestTokenCache_Get_Miss(t *testing.T) {
	...
	assert.False(t, ok)
	assert.Equal(t, testTokenResult{}, got)
}
...
func TestTokenCache_TTLExpiry(t *testing.T) {
	...
	tc := NewTokenCache[string, testTokenResult](ctx, 10, 0, 10*time.Millisecond)
...
func TestNewTokenCache_CleanupRespectsContextCancellation(t *testing.T) {
	...
	token := testTokenResult{AccessToken: "test", ExpiresIn: 1}
