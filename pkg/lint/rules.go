...
var KnownRules = map[string]RuleMeta{
	...
	"invalid-state-format": {
		...
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state backend's format is not a recognized value.",
		Why:         "format controls what shape of the state document a backend receives at save time. Only \"full\" (the default) and \"intent\" are recognized.",
		Fix:         "Set format to \"full\" or \"intent\", or omit it (defaults to \"full\").",
		Examples: []string{
			"state:\n  enabled: true\n  backend:\n    provider: file\n    format: intent\n    inputs:\n      path: \"intent.json\"",
		},
	},
	"missing-state-emit-backend": {
		Rule:        "missing-state-emit-backend",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.emit entry does not specify a backend provider.",
		Why:         "Each emit target writes through its own backend provider, exactly like the primary backend. Without a provider, there is nothing to save to.",
		Fix:         "Set provider on the emit entry to a registered provider with CapabilityState (e.g., 'file').",
	},
	"invalid-state-emit-backend": {
		Rule:        "invalid-state-emit-backend",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.emit entry references a provider that is not registered or lacks CapabilityState.",
		Why:         "Emit targets must implement CapabilityState, exactly like the primary backend. Using an unregistered or incompatible provider will fail at runtime.",
		Fix:         "Use a registered provider with CapabilityState such as 'file' or 'http'. External providers like 'github' require an installed plugin.",
	},
	"state-format-lossy-with-immutable": {
		Rule:        "state-format-lossy-with-immutable",
		Severity:    string(SeverityWarning),
		Category:    "state",
		Description: "The primary state backend saves in the lean \"intent\" format, but the solution has immutable resolvers.",
		Why:         "Immutable resolver locks live in the \"resolvers\" section, which the intent format omits, so cross-run immutable enforcement silently stops working against that backend alone -- a resolver value can drift between runs with no error.",
		Fix:         "Keep the primary backend in the default \"full\" format and move format: intent to an emit target instead (so a full-fidelity copy still exists locally), or remove immutable: true if that resolver's cross-run consistency does not need enforcing.",
		Examples: []string{
