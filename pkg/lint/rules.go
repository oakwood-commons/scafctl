...
var KnownRules = map[string]RuleMeta{
	...
	"parameter-numeric-matches": {
		...
		Why:         "A bare YAML number in a 'parameter' default (e.g. 'default: 1') stays an integer under auto. Integers have no matches() method in CEL, so the expression fails at runtime with a type error even though the solution passes lint. Quoting the default (e.g. 'default: \"1\"') keeps it a string under auto because an authored default retains its YAML type, and declaring 'type: string' also keeps it a string; other explicit types (e.g. raw, int) do not, since they preserve or coerce the value to a non-string.",
