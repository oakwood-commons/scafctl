...
var KnownRules = map[string]RuleMeta{
	...
	"unknown-resolver-reference": {
		...
		Severity:    string(SeverityError),
		Category:    "dependency",
		Description: "An unambiguous resolver reference (hard CEL access _.name / _[\"name\"], an explicit rslvr: reference, or an explicit template accessor {{ ._.name }} / {{ ._name }}) names a resolver that is not defined.",
		Why:         "The reference cannot resolve, so building the dependency graph fails at run time with \"depends on X but X wasn't present\". Reporting it at lint time surfaces the typo without executing the solution. Only unambiguous references are checked: bare Go-template accessors ({{ .field }}) are covered by template-unknown-accessor (which understands data-key and forEach-alias scope), and optional access (_.?name) by undefined-optional-reference. Engine-injected context keys are exempt: the entire `__` namespace (__actions, __item, __index, __error, __plan, ...) is reserved for the engine and is never a resolver name.",
	...
	"missing-state-load-provider": {
		Rule:        "missing-state-load-provider",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "The state.load block does not specify a provider.",
		Why:         "state.load reads state through a provider with CapabilityState (e.g., 'file' or 'github'). Without a provider there is nothing to read from.",
		Fix:         "Add a provider to the load block, e.g.:\n  state:\n    load:\n      provider: file\n      inputs:\n        path: \".state/state.json\"",
	},
	"invalid-state-load-provider": {
		Rule:        "invalid-state-load-provider",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "The state.load block references a provider that is not registered or lacks CapabilityState.",
		Why:         "State providers must implement CapabilityState. Using an unregistered or incompatible provider will fail at runtime.",
	...
	"invalid-state-format": {
		Rule:        "invalid-state-format",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.save target's format is not a recognized value.",
		Why:         "format controls what shape of the state document a save target receives. Only \"full\" (the default) and \"intent\" are recognized.",
		Fix:         "Set format to \"full\" or \"intent\", or omit it (defaults to \"full\").",
		Examples: []string{
			"state:\n  save:\n    - provider: file\n      format: intent\n      inputs:\n        path: \"intent.json\"",
		},
	},
	"missing-state-save-provider": {
		Rule:        "missing-state-save-provider",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.save target specifies neither a provider nor extends: load.",
		Why:         "Each save target writes through a provider with CapabilityState. Without a provider -- its own, or the load block's via extends: load -- there is nothing to write to.",
		Fix:         "Set provider on the save target, or use extends: load to write back to the load block's provider and inputs.",
		Examples: []string{
			"state:\n  load:\n    provider: file\n    inputs:\n      path: \".state/state.json\"\n  save:\n    - extends: load",
		},
	},
	"invalid-state-save-provider": {
		Rule:        "invalid-state-save-provider",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.save target references a provider that is not registered or lacks CapabilityState.",
		Why:         "Save targets must implement CapabilityState. Using an unregistered or incompatible provider will fail at runtime.",
		Fix:         "Use a registered provider with CapabilityState such as 'file' or 'http'. External providers like 'github' require an installed plugin.",
	},
	"invalid-state-save-extends": {
		Rule:        "invalid-state-save-extends",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.save target's extends is invalid: an unsupported value, combined with provider, or used without a state.load block.",
		Why:         "extends: load copies the load block's provider and inputs (the target's own inputs are merged on top). It cannot also name a provider, and there must be a load block to copy from. \"load\" is the only supported value.",
		Fix:         "Use extends: load together with a state.load block and without provider, or remove extends and set provider and inputs on the target.",
		Examples: []string{
			"# Load from main, save to a feature branch:\nstate:\n  load:\n    provider: github\n    inputs:\n      owner: { literal: acme }\n      repo: { literal: infra }\n      path: { literal: state/app.json }\n      ref: { literal: main }\n  save:\n    - extends: load\n      inputs:\n        branch: { rslvr: featureBranch }",
		},
	},
	"invalid-state-save-checkpoint": {
		Rule:        "invalid-state-save-checkpoint",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.save target sets checkpoint: true but does not use the \"full\" format.",
		Why:         "A checkpoint writes the target before workflow actions run so that immutable values locked this run survive a later action failure. Only a full-format document carries those locks; an intent document does not.",
		Fix:         "Remove checkpoint from the target, or move checkpoint: true to a full-format save target.",
		Examples: []string{
			"state:\n  load:\n    provider: file\n    inputs:\n      path: \".state/state.json\"\n  save:\n    - extends: load\n      checkpoint: true",
		},
	},
	"invalid-state-parameter-narrowing": {
		...
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.save target's parameters narrowing (include/exclude) is set, but its format is not \"intent\".",
		Why:         "Parameter narrowing only applies to the lean intent projection. Narrowing a \"full\" (or unset) target would silently drop parameters from the authoritative state document -- a value dropped from the saved record but still relied on for replay would go missing with no error.",
		Fix:         "Either remove the target's parameters narrowing, or set format: intent on that target.",
		Examples: []string{
	...
	"conflicting-state-parameter-narrowing": {
		...
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.save target's parameters narrowing sets both include and exclude.",
		...
		Fix:         "Set only one of parameters.include or parameters.exclude on the target.",
	},
	"immutable-requires-state": {
		Rule:        "immutable-requires-state",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A resolver has immutable: true but the solution has no state block, or one that configures neither load nor save.",
		Why:         "Without a state block that loads and saves there is nowhere to persist the locked resolver value, and nothing to verify it against on the next run. The immutable flag has no effect.",
		Fix:         "Add a state block with a load block and a full-format save target so that the resolver value can be persisted and verified.",
		Examples: []string{
			"state:\n  enabled: true\n  load:\n    provider: file\n    inputs:\n      path: \"my-state.json\"\n  save:\n    - extends: load\n\nspec:\n  resolvers:\n    cluster_id:\n      type: string\n      immutable: true\n      resolve:\n        with:\n          - provider: parameter\n            inputs:\n              key: \"cluster_id\"\n          - provider: exec\n            inputs:\n              command: \"uuidgen\"",
		},
	},
	"state-requires-load": {
		Rule:        "state-requires-load",
		Severity:    string(SeverityWarning),
		Category:    "state",
		Description: "The solution uses immutable resolvers or action fingerprints, but state has no load block.",
		Why:         "Immutable locks and fingerprints only work across runs if the saved document is read back. Without a load block every run starts from empty state, so locks are never verified and fingerprints never match.",
		Fix:         "Add a state.load block that reads the full state document a save target writes.",
		Examples: []string{
			"state:\n  load:\n    provider: file\n    inputs:\n      path: \".state/state.json\"\n  save:\n    - extends: load",
		},
	},
	"state-requires-full-save": {
		Rule:        "state-requires-full-save",
		Severity:    string(SeverityWarning),
		Category:    "state",
		Description: "The solution uses immutable resolvers or action fingerprints, but no state.save target writes the \"full\" format.",
		Why:         "Immutable locks and fingerprints are stored only in the full state document; an intent document omits them. Without a full-format save target they are never saved, so the next run cannot verify or reuse them.",
		Fix:         "Add a full-format save target -- for example extends: load -- alongside any intent-format targets.",
		Examples: []string{
			"state:\n  load:\n    provider: file\n    inputs:\n      path: \".state/state.json\"\n  save:\n    - extends: load          # full state, including locks\n    - provider: file\n      format: intent       # lean, signable copy\n      inputs:\n        path: \"intent.json\"",
		},
	},
	"immutable-without-checkpoint": {
		Rule:        "immutable-without-checkpoint",
		Severity:    string(SeverityInfo),
		Category:    "state",
		Description: "The solution has immutable resolvers and workflow actions, but no state.save target sets checkpoint: true.",
		Why:         "Without a checkpoint, state is written only after every action succeeds. If an action fails after creating something named from a freshly minted immutable value (a generated ID, say), that value is never saved and the next run mints a new one -- orphaning what the failed run created.",
		Fix:         "If that can happen, add checkpoint: true to a full-format save target so immutable values are written before actions run. Computed immutable values that a retry reproduces exactly do not need a checkpoint.",
		Examples: []string{
			"state:\n  load:\n    provider: file\n    inputs:\n      path: \".state/state.json\"\n  save:\n    - extends: load\n      checkpoint: true",
		},
	},
	"empty-state-config": {
		Rule:        "empty-state-config",
		Severity:    string(SeverityWarning),
		Category:    "state",
		Description: "The state block configures neither load nor save.",
		Why:         "state.load is the only way state is read and state.save the only way it is written. A state block with neither does nothing.",
		Fix:         "Add a state.load block, one or more state.save targets, or both -- or remove the state block.",
	},
	"state-ref-state-dependent": {
		Rule:        "state-ref-state-dependent",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.enabled or state.load.inputs field references a state-dependent resolver -- one that reads the state snapshot (via the state provider) or transitively depends on one that does.",
		Why:         "state.enabled and load inputs are evaluated in a pre-load pass, before the state snapshot exists. A state-dependent resolver cannot run in that pass, so referencing one is a circular dependency. References to state-INDEPENDENT resolvers are fine -- the engine resolves them before loading state. Save targets are resolved after every resolver has run, so they are not restricted.",
	...
	"state-ref-unknown": {
		...
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.enabled or state.load.inputs field references a resolver that is not defined in spec.resolvers.",
	...
	"state-save-state-ref": {
		Rule:        "state-save-state-ref",
		Severity:    string(SeverityError),
		Category:    "state",
		Description: "A state.save target's input or enabled condition references the state being saved (__state), creating a circular dependency.",
		Why:         "The state provider reads the state being saved, so a save target cannot depend on it. Save-target fields may otherwise reference any resolver, since every resolver has run by save time.",
	...
	"state-github-no-save-branch": {
		...
		Severity:    string(SeverityInfo),
		Category:    "state",
		Description: "A GitHub state.save target has no branch configured. For PR workflows, set the branch on the save target at save time.",
		Why:         "Without a branch, state is saved to the branch it was loaded from (typically main). For PR-based workflows, you likely want to save state to a feature branch derived from a resolver.",
		Fix:         "Add a branch input to the save target referencing a resolver:\n  save:\n    - extends: load\n      inputs:\n        branch: { rslvr: featureBranch }\nThis saves state to the same branch as your scaffolded files.",
	},
	"unused-suppression": {
	...
	"missing-template-dependency": {
		...
		Why:         "When a go-template render-tree step renders external template files, the DAG cannot auto-detect which resolver names are used inside those files. Discovery is role-based, not extension-based: any file reached via bundle.include or a directory provider is treated as a go-template source (e.g. .tpl, .tmpl, .gotmpl, but also .tf, .yaml, or any other extension), so references inside non-standard extensions are still detected. Files matched by the render-tree step's literal rawGlobs patterns are excluded, since raw entries are copied verbatim and never parsed for resolver references. Missing dependencies cause 'map has no entry for key' runtime errors because the template executes before its data dependencies are resolved.",
		Fix:         "Add the missing resolver names to the dependsOn list of the render-tree resolver, or of the resolver that consumes the rendered output. If a flagged file is meant to be copied verbatim, add its path to the render-tree step's rawGlobs so it is excluded from dependency scanning.",
		Examples: []string{
