...
- *(mcp)* Reduce MCP tool-schema and server-instructions context footprint: dedupe repeated `cwd` parameter descriptions and the `onConflict`/provider-capability enum literals into shared constants in `pkg/mcp/descriptions.go`, trim the `preview_resolvers` tool description from tutorial prose to a factual summary, and cut `serverInstructionsTemplate` by ~71% (14.7KB -> ~4.3KB) by replacing inlined tutorials (CEL variables, function catalogs, CLI examples, tool latency guide) with pointers to the equivalent on-demand MCP tools (`list_context_variables`, `list_cel_functions`, `get_provider_schema`, `explain_concepts`, etc). No tool behavior, schema values, or enum values changed (#729)
