...
- *(lint)* Add `unknown-resolver-reference` rule (error) to catch an unambiguous reference -- hard CEL access (`_.name`, `_["name"]`), an explicit `rslvr:` reference, or a `{{ ._.name }}` template accessor -- that names a resolver which is not defined. Previously such a typo only surfaced indirectly as an `unused-resolver` warning on the correctly-spelled resolver, or at run time when the dependency graph failed to build. Bare Go-template accessors remain the domain of `template-unknown-accessor` (which understands `data`-key and `forEach`-alias scope) and optional access (`_.?name`) of `undefined-optional-reference` (#696)
