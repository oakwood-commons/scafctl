...
When invoked via a command prompt (e.g. `completeness-check`), follow that
...
6. **Offer to implement the gaps.** End the response with a short numbered list
   ...
   Phrase this as an explicit follow-up question. Only after the user confirms
   should you implement the selected gaps -- then re-run the checklist to verify
   each is resolved.
